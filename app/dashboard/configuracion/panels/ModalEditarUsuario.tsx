...
export default function ModalEditarUsuario({ usuario, onClose, onUpdated }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const usuarioVal = (fd.get("usuario") as string).trim();
    ...
    const nuevaPassword = (fd.get("nuevaPassword") as string).trim() || null;

    if (!usuarioVal || !role) { setError("Usuario y rol son obligatorios."); return; }
    if (nuevaPassword && nuevaPassword.length < 6) { setError("La nueva contraseña debe tener al menos 6 caracteres."); return; }

    setLoading(true);
    try {
      const actualizado = await editarUsuario(usuario.id, { usuario: usuarioVal, nombre, apellido, role, estatus, nuevaPassword });
      onUpdated(actualizado as unknown as Usuario);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "";
      setError(msg.includes("Unique") || msg.includes("unique") ? "Ese nombre de usuario ya existe." : "Error al actualizar. Intenta de nuevo.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-gray-200 border border-gray-300 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-5 bg-blue-100 border-b border-blue-200 rounded-t-2xl flex-shrink-0">
          <h2 className="text-[#0d0d0d] font-bold text-lg">Editar usuario</h2>
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="px-6 py-6 space-y-4 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="nombre" type="text" maxLength={100} defaultValue={usuario.nombre ?? ""}
            ...
            <div>
              ...
              <input name="apellido" type="text" maxLength={100} defaultValue={usuario.apellido ?? ""}
                className="w-full bg-white border border-gray-300 rounded-xl px-4 py-2.5 text-gray-800 placeholder-gray-400 text-sm focus:outline-none focus:border-[#D4A017] focus:ring-1 focus:ring-[#D4A017]/50 transition-colors" />
            </div>
          </div>
          ...
          <div>
            ...
            <input name="usuario" type="text" required maxLength={50} defaultValue={usuario.usuario ?? ""}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select name="estatus" defaultValue={String(usuario.estatus)}
                className="w-full bg-white border border-gray-300 text-gray-800 text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-[#D4A017] focus:ring-1 focus:ring-[#D4A017]/50 transition-colors cursor-pointer">
          ...
          <div>
            <label className="block text-gray-700 text-sm font-medium mb-1.5">
              Nueva contraseña <span className="text-gray-400 font-normal">(opcional — déjala vacía para no cambiarla)</span>
            </label>
            <div className="relative">
              <input name="nuevaPassword" type={passVisible ? "text" : "password"} placeholder="Mínimo 6 caracteres"

...
interface Props {
  dulce: Dulce;
  onClose: () => void;
  onUpdated: (d: Dulce) => void;
  role: string;
}

export default function ModalEditarDulce({ dulce, onClose, onUpdated, role }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const descripcion = (fd.get("descripcion") as string).trim() || nombre;
    ...
    if (!nombre || !costoStr || !cantidadStr) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-gray-200 border border-gray-300 rounded-2xl shadow-2xl w-full max-w-md flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 bg-blue-100 border-b border-blue-200 rounded-t-2xl">
          <h2 className="text-[#0d0d0d] font-bold text-lg">Editar dulce</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              type="text"
              maxLength={200}
              defaultValue={dulce.descripcion}
              className="w-full bg-white border border-gray-300 rounded-xl px-4 py-2.5 text-gray-800 placeholder-gray-400 text-sm focus:outline-none focus:border-[#D4A017] focus:ring-1 focus:ring-[#D4A017]/50 transition-colors"
            />
          </div>
            ...
            <div>
              ...
              {puedeEditarInventario ? (
                <input
                  ...
                  type="number"
                  min={0}
                  defaultValue={dulce.cantidad}
                  className="w-full bg-white border border-gray-300 rounded-xl px-4 py-2.5 text-gray-800 text-sm focus:outline-none focus:border-[#D4A017] focus:ring-1 focus:ring-[#D4A017]/50 transition-colors"
                />
              ) : (
                <>
                  <input type="hidden" name="cantidad" value={dulce.cantidad} />
          ...
          <div>
            ...
            <select
              name="estatus"
              defaultValue={String(dulce.estatus)}
